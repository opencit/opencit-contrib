...
execPrereqCheck(){
  ...
  # fi
   ...
   if [ -z "$KLOCWORK_PROJECT" ]; then
      echo_failure "KLOCWORK_PROJECT Env. variable is missing (i.e. dcg_security-contrib)"
#      return 1
	check=1
   fi
...
#      return 1
   ...
   if [ -z "$MAIN_PROJECT_SPEC" ]; then
      echo_failure "MAIN_PROJECT_SPEC Env. variable is missing (i.e. contrib.out)"
#      return 1
	check=1
   fi
}
...
displayUsage() {
	...
	echo "Usage:"
	echo -e "This script allows you to run the klocwork scan for the contrib projects. The script needs the following environment variables set before running: \n"
	echo -e " *BUILD_SPECS_DIRECTORY (i.e. /home/klocwork/kw_build_specs) \n *TABLES_DIRECTORY (i.e. /home/klocwork/kwtables) \n *KLOCWORK_PROJECT (i.e. dcg_security-contrib) \n *KLOCWORK_SERVER_URL (i.e. https://klocwork-jf18.devtools.intel.com:8160) \n *MAIN_PROJECT_SPEC (i.e. contrib.out) \n"
